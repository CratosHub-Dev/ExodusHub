-- Safe Rayfield loader — prevents "attempt to call a nil value" if the URL
-- is temporarily down or returns a non-compilable response.
local Rayfield
do
    local URLS = {
        "https://sirius.menu/rayfield",           -- primary
        "https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/main/source.lua", -- fallback
    }
    local loaded, chunk, err
    for _, url in ipairs(URLS) do
        local ok, body = pcall(game.HttpGet, game, url)
        if ok and type(body) == "string" and #body > 100 then
            chunk, err = loadstring(body)
            if chunk then
    ...
    if not loaded then
        -- Surface a readable error instead of a silent "attempt to call a nil value"
        error("[Exodus] Failed to load Rayfield UI library.\nMake sure HTTP requests are allowed in your executor and try again.\nDetails: " .. tostring(err), 0)
    end
end
...
local RealHRP = nil
local InvisDefaultPos = UDim2.new(0.5, 180, 0.5, -90)

-- Forward declaration: ToggleGhostMode is defined later but InvisBtn's OnClick needs it.
-- Without this, the closure captures nil (global lookup fails) and clicking InvisBtn errors.
local ToggleGhostMode

local InvisGui = Instance.new("ScreenGui")
InvisGui.Name = "Exodus_FloatingInvis"
InvisGui.ResetOnSpawn = false
local invisSuccess, invisCore = pcall(function() return game:GetService("CoreGui") end)
InvisGui.Parent = invisSuccess and invisCore or lp:WaitForChild("PlayerGui")

local InvisBtn = CreateFloatingButton({
    Name = "InvisButton",
    Text = "INVIS: 🔴",
    Position = InvisDefaultPos,
    ...
    OnClick = function() ToggleGhostMode() end
})

local XrayDefaultPos = UDim2.new(0.5, 180, 0.5, -130)

-- XrayBtn: manual creation with stored stroke reference
local XrayState = false
local XrayBtn = Instance.new("TextButton")
XrayBtn.Name = "XrayButton"
XrayBtn.Parent = InvisGui
XrayBtn.Visible = false
XrayBtn.Size = UDim2.new(0, 90, 0, 30)
XrayBtn.Position = XrayDefaultPos
XrayBtn.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
XrayBtn.BackgroundTransparency = 0.3
XrayBtn.Text = "X-RAY: 🔴"
XrayBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
XrayBtn.Font = Enum.Font.GothamBold
XrayBtn.TextSize = 13
XrayBtn.TextTransparency = 0.1
XrayBtn.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
XrayBtn.TextStrokeTransparency = 0.5

local XrayCorner = Instance.new("UICorner")
XrayCorner.CornerRadius = UDim.new(0, 6)
XrayCorner.Parent = XrayBtn

local XrayStroke = Instance.new("UIStroke")
XrayStroke.Thickness = 2
XrayStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
XrayStroke.Color = Color3.fromRGB(255, 0, 0)
XrayStroke.Transparency = 0.2
XrayStroke.Parent = XrayBtn

-- Hover effects (tween only thickness/background, NEVER color)
XrayBtn.MouseEnter:Connect(function()
    TweenService:Create(XrayBtn, TweenInfo.new(0.3), {BackgroundTransparency = 0.1, TextTransparency = 0}):Play()
    TweenService:Create(XrayStroke, TweenInfo.new(0.3), {Thickness = 3}):Play()
end)

XrayBtn.MouseLeave:Connect(function()
    TweenService:Create(XrayBtn, TweenInfo.new(0.3), {BackgroundTransparency = 0.3, TextTransparency = 0.1}):Play()
    TweenService:Create(XrayStroke, TweenInfo.new(0.3), {Thickness = 2}):Play()
end)

-- XrayBtn click: use stored XrayStroke, fix map detection
XrayBtn.MouseButton1Click:Connect(function()
    XrayState = not XrayState
    if XrayState then
        XrayBtn.Text = "X-RAY: 🟢"
        XrayStroke.Color = Color3.fromRGB(0, 255, 0)
    else
        XrayBtn.Text = "X-RAY: 🔴"
        XrayStroke.Color = Color3.fromRGB(255, 0, 0)
    end
    -- Generic map detection
    for _, mapCandidate in pairs(workspace:GetChildren()) do
        if mapCandidate:IsA("Model") and mapCandidate:FindFirstChildWhichIsA("BasePart", true) then
            local basePartCount = 0
            for _, desc in pairs(mapCandidate:GetDescendants()) do
                if desc:IsA("BasePart") then
                    basePartCount = basePartCount + 1
                    if basePartCount > 50 then break end
                end
            end
            if basePartCount > 50 then
                for _, v in pairs(mapCandidate:GetDescendants()) do
                    if v:IsA("BasePart") and not v:IsDescendantOf(lp.Character) then
                        v.LocalTransparencyModifier = XrayState and 0.5 or 0
                    end
                end
                break
            end
...
ToggleGhostMode = function()
    ...
    if InvisState then
        InvisBtn.Text = "INVIS: 🟢"
        InvisBtn.UIStroke.Color = Color3.fromRGB(0, 255, 0)
    else
        InvisBtn.Text = "INVIS: 🔴"
        InvisBtn.UIStroke.Color = Color3.fromRGB(255, 0, 0)
    end
    ...
    if InvisState then
        local spawnPos = RealHRP.CFrame
        char.Archivable = true
        GhostCharacter = char:Clone()
        GhostCharacter.Name = "Exodus_Ghost"
        ...
        local ghostHead = GhostCharacter:FindFirstChild("Head")
        if ghostHead then
            ghostHead.Transparency = 1
            for _, child in pairs(ghostHead:GetChildren()) do
        ...
        if GhostCharacter:FindFirstChild("HumanoidRootPart") then
            GhostCharacter.PrimaryPart = GhostCharacter.HumanoidRootPart
        end
        ...
        RealHRP.CFrame = CFrame.new(0, 20000, 0)

        local ghostHRP = GhostCharacter:WaitForChild("HumanoidRootPart", 2)
        if ghostHRP then
            ghostHRP.CFrame = spawnPos
        end

        local ghostHum = GhostCharacter:WaitForChild("Humanoid", 2)
        if ghostHum then
            ghostHum.WalkSpeed = 16
            ghostHum.JumpPower = 50
            ghostHum.AutoRotate = true
        ...
        ghostLoopConnection = RunService.Heartbeat:Connect(function()
            if not InvisState or not GhostCharacter or not GhostCharacter.Parent then
        ...
        if GhostCharacter then
            local gHRP = GhostCharacter:FindFirstChild("HumanoidRootPart")
            if gHRP then
                ghostCF = gHRP.CFrame
            end
        ...
        if RealHRP and RealHRP.Parent then
            ...
            RealHRP.CFrame = ghostCF
        end

        if realHum then
            realHum.PlatformStand = false
            realHum:ChangeState(Enum.HumanoidStateType.GettingUp)
...
local function getCombatData()
    ...
    local originPos, targetPos
        ...
        -- Your new origin and target logic
        originPos = root.Position + Vector3.new(0, 0, -0.4)
...
local function fireGun()
    ...
    if root and originPos and targetPos then
        ...
        if remote then
            -- Convert Vector3 positions back to CFrame for the remote
            local originCF = CFrame.lookAt(originPos, targetPos)
...
local function serverHop(smallServer)
    ...
    if not success or not response or not response.data then
        Rayfield:Notify({Title = "Error", Content = "Failed to fetch servers.", Duration = 3})
        return
    end
    ...
    if #servers == 0 then
        Rayfield:Notify({Title = "Error", Content = "No other servers found.", Duration = 3})
        return
    end
    if smallServer then
        table.sort(servers, function(a, b) return a.playing < b.playing end)
    end
    TeleportService:TeleportToPlaceInstance(PlaceId, servers[1].id, lp)
end
...
local function holdHostage()
    if getMurderer() ~= lp then
        Rayfield:Notify({Title = "Exodus", Content = "You are not the Murderer!", Duration = 3})
        return
    end
        ...
        if _G.AutoCoins and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if coins then
                ...
                if target and target.Parent then
                    ...
                    if target and target.Parent then firetouchinterest(hrp, target, 1) end
                end
            end
        else
...
local function setKillAura(enabled)
    ...
    if enabled then
        killAuraConnection = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                tHRP.Anchored = true
                tHRP.CFrame = lp.Character.HumanoidRootPart.CFrame + lp.Character.HumanoidRootPart.CFrame.LookVector * 2
                task.wait(0.1)
            ...
            task.delay(0.3, function()
                if tHRP then tHRP.Anchored = false end
            end)
        end)
    end
end
...
local function killAll()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            tHRP.Anchored = true
            tHRP.CFrame = lp.Character.HumanoidRootPart.CFrame + lp.Character.HumanoidRootPart.CFrame.LookVector * 1
            table.insert(victims, tHRP)
        end
    end
    ...
    task.delay(0.5, function()
        for _, tHRP in ipairs(victims) do
            if tHRP then tHRP.Anchored = false end
        end
    end)
end

local function sendCoinWebhook(status, coins)
    if _G.WebhookURL == "" or _G.WebhookURL == nil then return end
    local data = {
        ["embeds"] = {{
            ["title"] = "✨ EXODUS HUB | COIN TRACKER 🪙",
            ["description"] = "**Status:** " .. status .. "\n**Collected:** `+" .. tostring(coins) .. "`\n**Account:** ||" .. lp.Name .. "||",
...
local function getLobbyCFrame()
    ...
    if lobby then
        ...
        if spawns and #spawns:GetChildren() > 0 then
            return spawns:GetChildren()[1].CFrame + Vector3.new(0, 5, 0)
        end
        for _, child in pairs(lobby:GetDescendants()) do
            if child:IsA("SpawnLocation") or child.Name:lower():find("spawn") then
                return child.CFrame + Vector3.new(0, 5, 0)
            end
        end
        return lobby:GetPivot() + Vector3.new(0, 5, 0)
    end
    ...
    return CFrame.new(-108.5, 145, 41)
end
...
local function isInLobby()
    ...
    local distance = (hrp.Position - lobbyCF.Position).Magnitude
    local container = getCoinContainer()
    if not container and distance < 300 then return true end
    return distance < 200
end

RunService.Heartbeat:Connect(function()
    if _G.SafeAutoFarm and _G.CoinTween then
        local container = getCoinContainer()
        if not container or not container.Parent or isInLobby() then
            _G.CoinTween:Cancel()
            _G.CoinTween = nil
...
RunService.Stepped:Connect(function()
    if _G.SafeAutoFarm and lp.Character then
        local hrp = lp.Character:FindFirstChild("HumanoidRootPart")
        local hum = lp.Character:FindFirstChild("Humanoid")
...
local function ResetFarmPhysics()
    ...
    if hum then
        ...
        hum.WalkSpeed = _G.DesiredSpeed or 16
        hum.JumpPower = _G.DesiredJump or 50
        hum.UseJumpPower = true
        hum.AutoRotate = true
    end
    if hrp then
...
task.spawn(function()
    ...
    while task.wait(0.1) do
        if not _G.SafeAutoFarm then continue end
        local char = lp.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local hum = char and char:FindFirstChild("Humanoid")
        ...
        local fullUI = mainGui and mainGui:FindFirstChild("Lobby") and mainGui.Lobby:FindFirstChild("Dock")
                       and mainGui.Lobby.Dock:FindFirstChild("CoinBags") and mainGui.Lobby.Dock.CoinBags:FindFirstChild("Container")
                       and mainGui.Lobby.Dock.CoinBags.Container:FindFirstChild("Coin")

        local isFull = fullUI and fullUI:FindFirstChild("Full") and fullUI.Full.Visible or false
        local liveCoinUI = fullUI and fullUI:FindFirstChild("CurrencyFrame") and fullUI.CurrencyFrame:FindFirstChild("Icon") and fullUI.CurrencyFrame.Icon:FindFirstChild("Coins")

        if liveCoinUI then
            local currentVal = tonumber(liveCoinUI.Text)
            if currentVal then
                coinsThisRound = currentVal
                if coinsThisRound == 0 then _G.HasReportedCoins = false end
            end
        end
        ...
        if inLobby then
            if not _G.HasReportedCoins and coinsThisRound > 0 then
                sendCoinWebhook("Round Ended ⚠️", coinsThisRound)
                _G.HasReportedCoins = true
            end
            if _G.CoinTween then
                _G.CoinTween:Cancel()
                _G.CoinTween = nil
        ...
        if isFull then
            ...
            if not _G.HasReportedCoins then
                sendCoinWebhook("Bag Full! ✅", coinsThisRound)
                _G.HasReportedCoins = true
            end
            continue
        end
        ...
        local container = getCoinContainer()
        if container and not isInLobby() then
            ...
            if targetCoin then
                ...
                _G.CoinTween = TweenService:Create(hrp, tweenInfo, {CFrame = targetCoin.CFrame * CFrame.new(0, -4, 0)})
                _G.CoinTween:Play()

                while _G.SafeAutoFarm and targetCoin.Parent == container and _G.CoinTween.PlaybackState == Enum.PlaybackState.Playing do
                    task.wait(0.05)
                    if fullUI and fullUI:FindFirstChild("Full") then
                        isFull = fullUI.Full.Visible
                    end
                    if isFull or isInLobby() then break end
                end
...
MainTab:CreateToggle({
    Name = "Show Prank Button",
    CurrentValue = false,
    Flag = "SPB_F",
    Callback = function(V) BombBtn.Visible = V end
})
MainTab:CreateToggle({
...
MainTab:CreateButton({
    ...
    Callback = function()
        ...
        InvisBtn.Position = InvisDefaultPos
        XrayBtn.Position = XrayDefaultPos
    end
})
...
local LastSessionBag = 0

local CoinTracker = FarmTab:CreateParagraph({
    Title = "Coin Live Tracker",
    Content = "Elapsed: 0d 0h 0m 0s\nBag: 0/0\nTotal Collected: 0"
})
...
task.spawn(function()
    while task.wait(1) do
        ...
        pcall(function()
            -- Accessing the text directly from your path
            local pGui = lp:FindFirstChild("PlayerGui")
            local mainGUI = pGui and pGui:FindFirstChild("MainGUI")
            ...
            if coinsLabel then
                coinText = coinsLabel.Text
                local current, max = coinText:match("(%d+)%D+(%d+)")
                bagValue = tonumber(current) or 0
                maxBag = tonumber(max) or 40
            end
        end)

        -- 3. Total Collected Logic
        if bagValue < LastSessionBag then
            TotalCollected = TotalCollected + LastSessionBag
        end
        LastSessionBag = bagValue

        -- 4. Final Display
        local displayBag = bagValue .. "/" .. maxBag
        CoinTracker:Set({
            Title = "Coin Live Tracker",
            Content = "Elapsed: " .. timeStr .. "\nBag: " .. displayBag .. "\nTotal Collected: " .. (TotalCollected + bagValue)
        })
    end
end)

FarmTab:CreateSection("Safe AFK System")
FarmTab:CreateToggle({
    Name = "Enable Safe Coin Farm",
    CurrentValue = false,
    Flag = "AutoCoinToggle",
    Callback = function(Value)
        ...
        if not Value then
            ...
            ResetFarmPhysics()
            task.delay(0.5, ResetFarmPhysics)
        end
    end,
})
...
flingDropdown = FlingTab:CreateDropdown({
    ...
    Callback = function(Option)
        if Option and Option[1] then
            selectedFlingTarget = Players:FindFirstChild(Option[1])
        end
    end,
})
...
local function makeDraggable(btn, action)
    ...
    UserInputService.InputChanged:Connect(function(input)
        -- Fixed: was MouseButton1 (press event), must be MouseMovement (move event)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            btn.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
...
-- Toggles _G.LockAll so all other buttons freeze in place.
-- This button is ALWAYS draggable regardless of lock state — otherwise you
-- could never move it after locking! It also syncs with the Rayfield toggle.
-- ─────────────────────────────────────────────────────────────────────────────
local LockBtnLocked   = false
local LockColor_Off   = Color3.fromRGB(255, 200, 0)   -- gold = unlocked
local LockColor_On    = Color3.fromRGB(255, 60,  60)  -- red  = locked

local LockBtn = Instance.new("TextButton")
LockBtn.Name            = "LockButton"
LockBtn.Parent          = ScreenGui
LockBtn.Visible         = true
LockBtn.Size            = UDim2.new(0, 44, 0, 44)
LockBtn.Position        = UDim2.new(0, 12, 0.5, -22)
...
LockBtn.TextSize        = 22
LockBtn.TextTransparency = 0.05
LockBtn.ZIndex          = 10  -- always on top
...
lockStroke.Color      = LockColor_Off
lockStroke.Transparency = 0.15
lockStroke.Parent     = LockBtn

-- Hover effects
LockBtn.MouseEnter:Connect(function()
    TweenService:Create(LockBtn, TweenInfo.new(0.25), {BackgroundTransparency = 0.05}):Play()
    TweenService:Create(lockStroke, TweenInfo.new(0.25), {Thickness = 4}):Play()
end)
LockBtn.MouseLeave:Connect(function()
    TweenService:Create(LockBtn, TweenInfo.new(0.25), {BackgroundTransparency = 0.3}):Play()
    TweenService:Create(lockStroke, TweenInfo.new(0.25), {Thickness = 3}):Play()
end)

-- Click: toggle lock state
LockBtn.MouseButton1Click:Connect(function()
    LockBtnLocked = not LockBtnLocked
    _G.LockAll    = LockBtnLocked
    if LockBtnLocked then
        LockBtn.Text      = "🔒"
        lockStroke.Color  = LockColor_On
        TweenService:Create(lockStroke, TweenInfo.new(0.2), {Color = LockColor_On}):Play()
    else
        LockBtn.Text      = "🔓"
        lockStroke.Color  = LockColor_Off
        TweenService:Create(lockStroke, TweenInfo.new(0.2), {Color = LockColor_Off}):Play()
    end
end)

-- Drag — always active, ignores _G.LockAll intentionally so you can
-- always reposition the lock button itself even while locked.
local lockDragging, lockDragStart, lockStartPos = false, nil, nil
LockBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1
    or input.UserInputType == Enum.UserInputType.Touch then
        lockDragging  = true
        lockDragStart = input.Position
        lockStartPos  = LockBtn.Position
    end
end)
UserInputService.InputChanged:Connect(function(input)
    if lockDragging
    and (input.UserInputType == Enum.UserInputType.MouseMovement
      or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        LockBtn.Position = UDim2.new(
            lockStartPos.X.Scale,  lockStartPos.X.Offset + delta.X,
            lockStartPos.Y.Scale,  lockStartPos.Y.Offset + delta.Y
        )
    end
end)
UserInputService.InputEnded:Connect(function(input)
...
-- Keep the lock button in sync if the Rayfield toggle changes _G.LockAll externally
task.spawn(function()
    while true do
        task.wait(0.5)
        if _G.LockAll ~= LockBtnLocked then
            LockBtnLocked    = _G.LockAll
            LockBtn.Text     = LockBtnLocked and "🔒" or "🔓"
            lockStroke.Color = LockBtnLocked and LockColor_On or LockColor_Off
        end
    end
end)
